    ...
    def _compute_current_balance(self):
        for rec in self:
            approved_spent = sum(
                ...
                if l.move_type == "expense" and l.state in ("approved", "reimbursed")
            )
            replenished = sum(
                l.amount for l in rec.line_ids
                if l.move_type == "replenishment" and l.state == "reimbursed"
            )
            pending = sum(
                1 for l in rec.line_ids
                if l.move_type == "expense" and l.state in ("draft", "pending")
            )
